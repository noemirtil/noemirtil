...
def test_register(client, app):
    # The register view should render successfully on GET. client.get() makes a GET request and returns the Response object returned by Flask.To test that the page renders successfully, a simple request is made and checked for a 200 OK status_code. If rendering failed, Flask would return a 500 Internal Server Error code.
    assert client.get('/auth/register').status_code == 200
    # client.post() makes a POST request, converting the data dict into form data.
    response = client.post(
        '/auth/register', data={'username': 'a', 'password': 'a'}
    )
    # headers will have a Location header with the login URL when the register view redirects to the login view. On POST with valid form data, it should redirect to the login URL
    assert response.headers["Location"] == "/auth/login"

    with app.app_context():
        # the user’s data should be in the database.
